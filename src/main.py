...
def create_indicator(indicator: schemas.HealthIndicatorCreate, db: Session = Depends(database.get_db)):
    # This endpoint triggers Risk Engine and Follow-up generation automatically
    return crud.create_patient_indicator(db=db, indicator=indicator)

# --- Follow-up Endpoints ---

@app.get("/followups/", response_model=List[schemas.FollowUp])
def read_follow_ups(status: Optional[str] = None, skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db)):
...
def update_follow_up(follow_up_id: int, follow_up_update: schemas.FollowUpUpdate, db: Session = Depends(database.get_db)):
